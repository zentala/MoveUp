...
- **One credited counter** (E015): `SessionState.sitting_seconds`, exposed as `limit_used_secs` / `limitUsedSecs`, is the only value a timer, progress bar, colour band or sit-limit alert may read. It is never reset by a return to sitting. `sitting_seconds_total` and `standing_seconds` are raw daily totals for KPI — compare raw with raw, never raw with credited. `secs_since_last_break` exists for the Debug tab only.
- **Session Break Credit** (ADR 008): proportional — each second of break cancels `break_credit_multiplier` seconds of sitting: **3.0 in `standard`**, 2.0 in `default`, `relaxed`, `strict` and `demo`. Breaks < `break_min_secs` (default 60s) get no credit. The credit applied on each transition is stored on the session row (`break_credit` column).
...
- **PostureBalance notification**: only fires when `sitting_seconds_total >= 6h` AND `sitting_seconds_total > standing_seconds * 2` — both sides raw (E015-T03). Comparing the credited counter against raw standing time made the notification unreachable for exactly the users who take breaks. The 6h floor prevents a false "sitting most of today" after short sessions.
