...
## Goal
Show a ↑ or ↓ indicator next to today's sitting time, comparing to yesterday's total.
...
// Show ↓ (green) if sitting less, ↑ (amber) if sitting more
const deltaSymbol = sitDelta < -300 ? "↓" : sitDelta > 300 ? "↑" : "";
...
- Vitest: ↓ shown when sitting_secs < yesterday_sitting_secs - 300
- Vitest: ↑ shown when sitting_secs > yesterday_sitting_secs + 300
- Vitest: no arrow shown when difference < 300s

## Acceptance criteria
- [ ] ↓ (green) shown when sitting less than yesterday
- [ ] ↑ (amber) shown when sitting more than yesterday
