...
.claude/tasks/
└── 0001-test-framework.md          (this file)
...
| File | Changes |
|------|---------|
| `src-tauri/src/commands.rs` | Added `inject_reading` command + `Emitter` import |
| `src-tauri/src/lib.rs` | Registered `inject_reading` in invoke_handler (cfg-gated) |
| `src-tauri/src/session.rs` | Fixed test: `should_send_praise_halfway_fires_once_per_day` |
| `package.json` | Added 6 new test scripts, Playwright dependency |
| `vite.config.ts` | Added coverage thresholds |
| `CLAUDE.md` | Expanded Testing section with 3-layer strategy |
...
```bash
pnpm test:all
pnpm build          # Automatically runs tests
pnpm tauri:build    # Automatically runs tests
```
...
1. **Run integration tests**:
   ...
   - May need to adjust timing and Tauri API mocking

2. **Run E2E tests**:
   - Install: `cargo install tauri-driver`
   - Start driver: `tauri driver --compatibility-mode`
   - Run: `pnpm test:e2e`

3. **Fill in remaining TypeScript tests**:
   - Finish component tests (ProgressBar, SettingsPanel, etc.)
   - Finish hook tests (useDesk)
   - Finish util tests (format)

4. **Add more Rust tests if needed**:
   - Serial reader tests (currently minimal)
   - Activity tracker tests (currently minimal)

5. **Monitor coverage**: Aim for >85% if possible, but 80% is the gate.

## Notes

- Tests are NOT run on pre-commit (too slow for large suites)
- Tests ARE run before every build (prevents shipping broken code)
- Integration tests require the Tauri app to be running (`pnpm tauri:dev`)
- E2E tests require `tauri-driver` (install once)
- All timestamps in this document use UTC (Rust testing convention)
