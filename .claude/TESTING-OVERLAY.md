...
```
⏱️ [TIMER] frame_count=60, demo_mode=true, visible=true
⏱️ [TIMER] frame_count=120, demo_mode=true, visible=true
⏱️ [TIMER] frame_count=180, demo_mode=true, visible=true
```
...
1. **Is demo_mode=true?** (check ⏱️ [TIMER] line)
...
Example description:
> "Bar is visible at top. It's always 100% width (full screen). Flickers every ~500ms. Stays white."

OR

> "Bar visible. Slowly grows: first 5 sec empty, then grows to 25%, keeps growing to 50%, etc. No flickering."

## Step 5: Analysis

Based on logs + visual description, we can determine:
- ✅ Demo mode working or ❌ not working
- ✅ Frame count incrementing or ❌ stuck
- ✅ Stage cycling or ❌ stuck on stage 0
- ✅ Progress overriding or ❌ stuck at 100%
- ✅ Rendering happens or ❌ frozen

Then fix based on actual evidence, not guessing.

---

## For Testing Visual Changes (Screenshots)

Once logging confirms values are changing, use Playwright to capture screen:

```bash
# In separate terminal, run test that screenshots top bar every 500ms for 30 seconds
pnpm test:overlay-screenshots
```

This will create: `.claude/overlay-screenshots/frame-0.png`, `frame-1.png`, etc.
Compare frame-by-frame to see bar width progression.
