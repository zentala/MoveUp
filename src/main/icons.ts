...
export async function generateIcons() {
  ...
  const whiteIconSVG = trayIconSVG.replace(
    /<path/g,
    '<path fill="#FFFFFF" stroke="#FFFFFF"'
  );
  const blackIconSVG = trayIconSVG.replace(
    /<path/g,
    '<path fill="#000000" stroke="#000000"'
  );

  const whiteIconPNG = await sharp(Buffer.from(whiteIconSVG))
    .png()
    .resize(16, 16)
    .toBuffer();
  const blackIconPNG = await sharp(Buffer.from(blackIconSVG))
    .png()
    .resize(16, 16)
    .toBuffer();
