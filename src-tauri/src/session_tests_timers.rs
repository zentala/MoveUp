...
mod timer_tests {
    ...
    fn transition_to_walking(m: &mut SessionManager) {
        for _ in 0..DEBOUNCE_COUNT {
    ...
    #[test]
    fn t042_09_standing_to_walking_break_continues() {
        let mut m = SessionManager::new();
        transition_to_sitting(&mut m);
        m.state.sitting_started = Some(Utc::now());
        transition_to_standing(&mut m);
        ...
        m.state.break_started = Some(Utc::now() - chrono::Duration::seconds(60));
        let live = m.get_live_break_seconds(Utc::now());
        assert!(live >= 59 && live <= 61, "break should grow in Walking, got {}", live);
    }

    #[test]
    fn t042_10_walking_to_sitting_applies_credit() {
        let mut m = SessionManager::new();
        transition_to_sitting(&mut m);
    ...
    #[test]
    fn t042_10b_standing_walking_sitting_accumulates_standing() {
        let mut m = SessionManager::new();
        transition_to_sitting(&mut m);
        m.state.sitting_started = Some(Utc::now());
