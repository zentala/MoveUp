...
fn f(name: &str, ty: &str, desc: &str) -> FieldMeta {
    FieldMeta { name: name.into(), ty: ty.into(), description: desc.into() }
}

fn sensor_source() -> DataSource {
    DataSource {
        id: "sensor".into(),
...
fn sqlite_sessions_source() -> DataSource {
    DataSource {
...
fn snapshots_source() -> DataSource {
    DataSource {
        id: "snapshots".into(),
...
fn events_log_source() -> DataSource {
    DataSource {
        id: "events_log".into(),
        name: "Event log".into(),
        kind: "log".into(),
...
fn profiles_ergonomic_source() -> DataSource {
    DataSource {
...
fn profiles_communication_source() -> DataSource {
    DataSource {
...
fn remote_ws_source() -> DataSource {
    DataSource {
        id: "remote_ws".into(),
...
mod tests {
    ...
    fn every_source_has_id_and_fields() {
        ...
        for src in &catalog.sources {
            ...
            assert!(
                !src.fields.is_empty(),
                "source {} has zero fields",
                src.id
            );
            for field in &src.fields {
                assert!(!field.name.is_empty(), "field in {} has empty name", src.id);
                assert!(!field.ty.is_empty(), "field {} in {} has empty type", field.name, src.id);
            }
        }
    }
